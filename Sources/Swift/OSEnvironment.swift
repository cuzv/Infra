...
public struct OSEnvironment {
    ...
    public static let isAppStoreReceiptSandbox: Bool = Bundle.main.appStoreReceiptURL?.lastPathComponent == "sandboxReceipt"
...
extension OSEnvironment {
    ...
    private static let hasSCInfoFolder: Bool = {
        #if os(iOS) || os(tvOS) || os(watchOS)
        return FileManager.default.fileExists(atPath: Bundle.main.bundleURL.appendingPathComponent("SC_Info").relativePath)
        #elseif os(macOS)
        return false
        #endif
    }()
    ...
    private static let hasEmbeddedMobileProvision: Bool = {
        #if os(iOS) || os(tvOS) || os(watchOS)
        return !(Bundle.main.path(forResource: "embedded", ofType: "mobileprovision") ?? "").isEmpty
        #elseif os(macOS)
        return false
        #endif
    }()
